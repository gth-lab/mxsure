...
mxsure_estimate <- function(mixed_snp_dist, unrelated_snp_dist, mixed_time_dist=NA, mixed_sites=NA, right_truncation=2000,threshold_time= NA,
                            ...
                            lambda_bounds=c(0, 1), k_bounds=c(0,1), intercept_bounds=c(-Inf, Inf), tree_fulldist_param_bounds = c(0, Inf),
      ...
      compute_shared_snps <- function(tip1, tip2, snp_dist, time_diff) {
        for (tree_i in tree_list) {
          ...
          if (tip1 %in% tips && tip2 %in% tips) {
            ...
            mrca_to_tip1 <- edge_dist[mrca_node, tip1_node] #min(c( edge_dist[mrca_node, tip1_node], edge_dist[mrca_node, tip2_node]))
            mrca_to_tip2 <- edge_dist[mrca_node, tip2_node] #max( c( edge_dist[mrca_node, tip1_node], edge_dist[mrca_node, tip2_node]))
      ...
      if(!anyNA(mixed_time_dist)&(anyNA(mixed_sites))){
        if ((length(mixed_snp_dist) >= 30) && (length(unrelated_snp_dist) >= 30)){
          ...
          if(anyNA(start_params)){
            ...
            start_vals <- expand.grid(k = c(0.25, 0.5, 0.75), lambda = c(0.01, 0.1, 1), intercept = c(0), tree_fulldist_mu=c(1e-10), tree_fulldist_size=c(1))
